...
import { ActionFormationParticipant, Participant } from "@/types"; 
...
export default function AssignExistingParticipantsDialog({
  ...
  const availableParticipants = useMemo(() => {
    ...
    const assignedIds = new Set(
      assignedParticipants.map((p) => p.participant_id)
    );
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Sélectionnez les participants à ajouter à cette session de
            formation.
          </DialogDescription>
        </DialogHeader>
        ...
        {errorAll && (
          <p className="text-red-500">Erreur de chargement des participants.</p>
        )}
                ...
                <div className="space-y-4">
                  {availableParticipants.map((participant) => (
                    ...
                    >
                      ...
                      />
                      <Label
                        htmlFor={`participant-${participant.user.user_id}`}
