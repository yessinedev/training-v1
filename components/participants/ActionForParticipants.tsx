...
const ActionForParticipants = ({actionId}: Props) => {
    ...
    const { data: participants = [] } = useQuery({
        ...
        queryFn: async () => {
          const response = await axiosInstance.get(`/formations/${actionId}/participants`);
      ...
      const removeParticipantMutation = useMutation({
        mutationFn: async ({ participantId }: { participantId: number }) => {
          await axiosInstance.delete(`/formations/${actionId}/participants?participantId=${participantId}`);
        },
        onSuccess: () => {
          queryClient.invalidateQueries({ queryKey: ["formation-participants", actionId] });
        },
      });
  return (
    <Card>
